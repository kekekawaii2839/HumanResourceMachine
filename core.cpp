...
bool Command::add_command(std::string a) {
	...
	std::string c(a, 0, i);
	int arg = -1;
	if (i < a.length() - 1) arg = a[i + 1] - 48;//目前认为参数为一位数字 
	if ("inbox" == c) {
...
int main() {
	...
	if (commands.is_open()) {
		int time = 0;
		while (!commands.eof()) {//逐行对指令集文件进行读取并处理
			commands.getline(buffer, 100);
			//std::cout << buffer << std::endl;
			if (time != 0) {
				bool f = command->add_command(buffer);
				if (!f) {
					std::cout << "“Error on instruction " << time + 1 << std::endl;
					return 3;
				}
			}
			else {
				...
				for (int i = length - 1; i >= 0; i--) {
					c += (int(buffer[i]) - 48) * pow(10, length - 1 - i);
				}
				std::cout << "c=" << c << std::endl;
				command->setCounter(c);
			}
			time++;
		}
	}
